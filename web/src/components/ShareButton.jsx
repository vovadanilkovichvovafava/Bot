...
export default function ShareButton({ text, title = 'Share Prediction', variant = 'button' }) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      const result = await sharePrediction(text, title);
  ...
  const handleCopyLink = async () => {
    await sharePrediction(text, title);
    setCopied(true);
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-900">Share Prediction</h3>
