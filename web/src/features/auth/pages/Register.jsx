...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!agreed) { setError(t('auth.errAgreeTerms', { defaultValue: 'Please accept the Terms & Privacy Policy' })); return; }
    ...
    <div className="min-h-[100dvh] flex flex-col overflow-y-auto" style={{ background: 'radial-gradient(120% 80% at 50% 0%, #16264f 0%, #0b1733 55%, #0a1430 100%)' }}>
      <div className="w-full max-w-sm mx-auto px-5 pt-6 pb-8 flex-1 flex flex-col">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Terms */}
          <button type="button" onClick={() => setAgreed(!agreed)} className="flex items-start gap-2.5 text-left w-full">
            <span className={`w-4 h-4 mt-0.5 rounded border flex items-center justify-center shrink-0 ${agreed ? 'bg-emerald-500 border-emerald-500' : 'border-white/25'}`}>
              {agreed && <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>}
            </span>
            <span className="text-white/50 text-xs leading-snug">{t('auth.iAgreeTerms', { defaultValue: 'I agree to the Terms & Conditions and Privacy Policy.' })}</span>
          </button>
