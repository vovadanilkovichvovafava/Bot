...
export default function ProAccess() {
  ...
  const [calcSel, setCalcSel] = useState(1); // default: 100€
      ...
      <div className={`q-step${step === 4 ? ' active' : ''}`}>
        <div className="q-body">
          <div className="q-bonus-header">
            <div className="q-bh-tag"><div className="q-bh-dot" />Только первый депозит</div>
            <div className="q-bh-title">Получи до <span style={{color:'#F7C948'}}>€750</span> бонуса<br/>бесплатно</div>
            <div className="q-bh-sub">Букмекер даёт +150% к первому депозиту.<br/>Это твои деньги — используй для ставок.</div>
          </div>
          ...
          <div className="q-calc-wrap">
            <div className="q-calc-opts">
              {[
                { dep: '€50',  bonus: '€75',   total: '€125',  lbl: 'Старт' },
                { dep: '€100', bonus: '€150',  total: '€250',  lbl: 'Популярно' },
                { dep: '€300', bonus: '€450',  total: '€750',  lbl: 'Выгодно', rec: true },
                { dep: '€500', bonus: '€750',  total: '€1250', lbl: 'Максимум' },
              ].map((opt, i) => (
                <div
                  key={i}
                ...
                >
                  {opt.rec && <div className="co-rec">🔥 Топ</div>}
                  <div className="co-dep">{opt.dep}</div>
                  <div className="co-lbl">{opt.lbl}</div>
                </div>
              ))}
            </div>
            <div className="q-calc-result">
              {[
                { dep: '€50',  bonus: '€75',   total: '€125',  months: '1 месяц' },
                { dep: '€100', bonus: '€150',  total: '€250',  months: '2 месяца' },
                { dep: '€300', bonus: '€450',  total: '€750',  months: '5 месяцев' },
                { dep: '€500', bonus: '€750',  total: '€1.250',months: '8 месяцев' },
              ].map((opt, i) => calcSel === i && (
                <div key={i}>
                  <div className="q-cr-row">
                    <div className="q-cr-label">Твой депозит</div>
                    <div className="q-cr-val">{opt.dep}</div>
                  </div>
                  <div className="q-cr-bonus">
                    <div className="q-cr-bonus-ico">🎁</div>
                    <div className="q-cr-bonus-body">
                      <div className="q-cr-bonus-title">+{opt.bonus} бонуса от букмекера</div>
                      <div className="q-cr-bonus-sub">Зачисляется автоматически после депозита</div>
                    </div>
                  </div>
                  <div className="q-cr-total">
                    <div className="q-cr-total-lbl">Итого банкролл для ставок</div>
                    <div className="q-cr-total-num">{opt.total}</div>
                    <div className="q-cr-total-sub">на счету — твои деньги, выводи когда хочешь</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="q-once-box">
            ...
            <div className="q-once-text">
              <strong>Акция действует только на первый депозит.</strong> После — бонус сгорает навсегда. У тебя есть один шанс — используй его по максимуму.
            </div>
          </div>
        </div>
        <div className="q-foot">
          <button className="q-btn gold" onClick={next}>
            Хочу максимум бонуса <ArrowRight />
          </button>
          <div className="q-hint">Чем больше депозит — тем больше зарабатываешь</div>
        </div>
      </div>
