...
class MatchAnalyzer:
    ...
    async def ai_chat(self, message: str, match_context: str = "", history: list = None, locale: str = "en") -> str:
        ...
        system = (
            ...
            "CRITICAL RULES:\n"
            "- ONLY analyze matches that appear in the [Real-time match data] context. "
            "NEVER invent, fabricate, or guess match fixtures. If no matches are listed for a league or date, "
            "explicitly tell the user that no matches are scheduled and suggest they check the next matchday.\n"
            "- Do NOT hallucinate scores, statistics, odds, or team lineups. Use ONLY the data provided.\n"
            "- If no real-time data is provided at all, clearly state that you don't have current match data "
            "and can only provide general football knowledge.\n\n"
            "Guidelines:\n"
            "- When real-time match data is provided, use it as the PRIMARY basis for your analysis.\n"
