...
async def get_traffic_analytics(
    ...
    try:
        ...
        daily_rows = (await db.execute(text("""
            ...
            FROM users
            WHERE created_at >= :since
            GROUP BY created_at::date, COALESCE(traffic_source, 'prescoreai')
        ...
        week_rows = (await db.execute(text("""
            SELECT COALESCE(traffic_source, 'prescoreai') AS src, COUNT(*) AS cnt
        ...
        country_rows = (await db.execute(text("""
            ...
            GROUP BY COALESCE(traffic_source, 'prescoreai'), country
            ORDER BY cnt DESC
            LIMIT 30
        """))).all()
