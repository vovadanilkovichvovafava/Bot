...
# ── Won accumulator slips ("чеки") ────────────────────────────────────────────
# Social proof for the home screen: winning accumulators built from REAL finished
# fixtures, so every team, score and market on the slip actually happened
# yesterday. Only the stake and the bookmaker branding are ours.
#
# Odds are derived from what the result was, using realistic ranges per market —
# API-Football does not keep historical odds for settled fixtures, and inventing
# a flat number per leg would look fake. High-odds markets (exact score, away
# underdog, big totals) are preferred so the accumulator lands in the 10x–60x
# range Vlad asked for.

# Bread-and-butter markets — most legs come from here so the accumulator stays
# in a believable range. A slip that turns €10 into €4000 reads as a scam.
_SAFE_MARKETS = [
    ("{home} win", lambda hg, ag: hg > ag, (1.55, 2.20)),
    ("Both teams to score", lambda hg, ag: hg > 0 and ag > 0, (1.70, 2.15)),
    ("Over 2.5 goals", lambda hg, ag: hg + ag >= 3, (1.65, 2.10)),
    ("Under 3.5 goals", lambda hg, ag: hg + ag <= 3, (1.50, 1.95)),
    ("{home} or draw", lambda hg, ag: hg >= ag, (1.35, 1.80)),
    ("{away} or draw", lambda hg, ag: ag >= hg, (1.45, 1.95)),
]

# Exactly one leg per slip comes from here — the "hook" that lifts the payout.
_VALUE_MARKETS = [
    ("{away} win", lambda hg, ag: ag > hg, (2.60, 4.60)),
    ("Over 3.5 goals", lambda hg, ag: hg + ag >= 4, (2.40, 3.90)),
    ("Over 4.5 goals", lambda hg, ag: hg + ag >= 5, (3.50, 5.80)),
    ("Draw", lambda hg, ag: hg == ag, (3.10, 4.40)),
    ("BTTS + Over 2.5", lambda hg, ag: hg > 0 and ag > 0 and hg + ag >= 3, (2.45, 3.60)),
]


def _leg_odds(seed: int, lo: float, hi: float) -> float:
    """Stable pseudo-random odds inside a range — same day → same slip."""
    span = int((hi - lo) * 100) or 1
    return round(lo + ((seed * 9301 + 49297) % span) / 100.0, 2)


@router.get("/won-slips")
async def won_slips(
    slips: int = Query(4, ge=1, le=8, description="How many accumulator slips to return"),
    legs: int = Query(4, ge=2, le=6, description="Fixtures per accumulator"),
    days_back: int = Query(1, ge=1, le=7, description="How far back to look for finished fixtures"),
):
    """Winning accumulators assembled from real finished fixtures.

    Public, read-only, never touches user data. Rotates daily: the seed is the
    date, so the carousel shows a fresh set of slips every day but stays stable
    within the day (no reshuffle on every render).
    """
    from app.services.api_football import api_football

    day = datetime.now() - timedelta(days=days_back)
    seed_base = day.year * 10000 + day.month * 100 + day.day

    try:
        fixtures = await api_football.get_fixtures_by_date(day.strftime("%Y-%m-%d"))
    except Exception as e:
        logger.warning("won-slips: failed to fetch fixtures: %s", e)
        return {"slips": [], "date": day.date().isoformat()}
    ...
    for f in fixtures or []:
        ...
        if status in ("FT", "AET", "PEN") and hg is not None and ag is not None and home and away:
            finished.append({
                "home": home, "away": away, "hg": int(hg), "ag": int(ag),
                "league": (f.get("league") or {}).get("name") or "",
            })

    if len(finished) < legs:
        logger.info("won-slips: only %d finished fixtures, need %d", len(finished), legs)
        return {"slips": [], "date": day.date().isoformat()}

    # Deterministic shuffle so the same day always yields the same slips
    finished.sort(key=lambda m: (hash((m["home"], m["away"])) ^ seed_base) & 0xFFFFFF)

    STAKES = [10, 15, 20, 25, 30]
    out = []
    cursor = 0
    for i in range(slips):
        if cursor + legs > len(finished):
            break
        chunk = finished[cursor:cursor + legs]
        cursor += legs

        # One leg carries the value market, the rest stay conservative — keeps
        # the total in the 12x–50x band that still looks achievable.
        hook_idx = (seed_base + i) % len(chunk)

        slip_legs, total_odds = [], 1.0
        for j, m in enumerate(chunk):
            hg, ag = m["hg"], m["ag"]
            pool = _VALUE_MARKETS if j == hook_idx else _SAFE_MARKETS
            options = [(lbl, rng) for lbl, pred, rng in pool if pred(hg, ag)]
            if not options:  # result fits nothing in this pool — fall back
                options = [(lbl, rng) for lbl, pred, rng in _SAFE_MARKETS if pred(hg, ag)]
            if not options:
                continue
            lbl, rng = options[(seed_base + i * 3 + j) % len(options)]
            odd = _leg_odds(seed_base + i * 17 + j * 5, rng[0], rng[1])
            total_odds *= odd
            slip_legs.append({
                "home": m["home"],
                "away": m["away"],
                "score": f"{hg}:{ag}",
                "league": m["league"],
                "market": lbl.format(hg=hg, ag=ag, home=m["home"], away=m["away"]),
                "odds": odd,
            })
        ...
        out.append({
            ...
            "stake": stake,
            "payout": round(stake * total_odds),
            "date": day.date().isoformat(),
        })

    return {"slips": out, "date": day.date().isoformat()}
