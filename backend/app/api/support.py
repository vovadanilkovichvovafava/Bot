...
def find_relevant_knowledge(message: str, full_history: str = "") -> str:
    ...
    keywords_map = {
        "getting_started": ["how to start", "getting started", "what do i do", "how to use", "where to begin",
                            ...
                            "jak zacząć", "jak używać", "co robić", "начать", "как начать", "что делать", "с чего начать", "как пользоваться",
                            "cómo empezar", "cómo funciona", "cómo uso", "qué hago", "como começar", "como funciona", "como uso", "o que faço",
                            "comment commencer", "comment ça marche", "comment utiliser", "par où commencer"],
        "ai_chat": ["ai chat", "ai prediction", "prediction", "forecast", "analysis", "analyze",
                     "previsioni", "previsione", "vorhersage", "prognoz", "прогноз", "анализ", "предсказание", "лучшие ставки",
                     "ask about match", "best bets", "today's picks", "recommend",
                     "predicción", "pronóstico", "análisis", "mejores apuestas", "previsão", "análise", "melhores apostas",
                     "prédiction", "pronostic", "analyse", "meilleurs paris"],
        "matches_page": ["matches", "fixtures", "today's games", "schedule", "partite", "spiele",
                         "mecze", "матчи", "матчи", "расписание", "league", "lega", "liga",
                         "partidos", "calendario", "jogos", "partidas", "matchs", "calendrier"],
        "live_matches": ["live", "ongoing", "real-time", "current match", "in diretta", "dal vivo",
                         "na żywo", "livescore", "лайв", "в прямом эфире", "en vivo", "en directo", "ao vivo", "en direct"],
        "value_finder": ["value bet", "value finder", "edge", "expected value", "ev",
                         "scan odds", "positive ev", "apuesta de valor", "aposta de valor", "pari de valeur"],
        "kelly_calculator": ["kelly", "bet size", "stake", "optimal bet", "bankroll fraction",
                             "quanto scommettere", "wie viel wetten", "ile postawić",
                             "сколько ставить", "размер ставки", "cuánto apostar", "quanto apostar", "combien parier", "mise optimale"],
        "bankroll_tracker": ["bankroll", "tracker", "balance", "roi", "profit", "loss",
                             "track bets", "track money", "банкролл", "баланс",
                             "saldo", "ganancias", "lucro", "solde", "bénéfice"],
        "bet_slip_builder": ["bet slip", "accumulator", "parlay", "multi bet", "combo",
                             "schedina", "kombiwette", "kupon", "экспресс", "купон",
                             "combinada", "cupón", "múltipla", "bilhete", "combiné", "pari combiné"],
        "odds_converter": ["odds convert", "decimal", "fractional", "american odds",
                           "implied probability", "convert odds", "odds format", "конвертер", "десятичные",
                           "convertir cuotas", "cuota decimal", "converter odds", "convertir les cotes", "cote décimale"],
        "your_stats": ["my stats", "statistics", "accuracy", "streak", "win rate",
                       "track record", "how good", "statistiche", "statistiken", "statystyki",
                       "статистика", "точность", "estadísticas", "precisión", "estatísticas", "precisão", "statistiques", "précision"],
        "prediction_history": ["history", "past predictions", "previous", "prediction history",
                               "storico", "verlauf", "historia", "история прогнозов", "история",
                               "historial", "histórico", "historique"],
        "settings": ["settings", "preferences", "risk level", "notifications", "alerts",
                     "impostazioni", "einstellungen", "ustawienia", "настройки", "уведомления",
                     "risk", "риск", "odds range", "min odds", "max odds",
                     "ajustes", "configuración", "notificaciones", "nivel de riesgo",
                     "configurações", "notificações", "paramètres", "niveau de risque"],
        "referral_program": ["referral", "refer", "invite", "friend", "bonus request",
                             "referral code", "share link", "invita", "einladen", "zaproś",
                             "реферал", "пригласить", "друг", "referido", "invitar", "amigo",
                             "indicação", "convidar", "parrainage", "inviter", "ami"],
        "pro_access": ["pro", "premium", "unlock", "upgrade", "unlimited",
                       "sblocca", "freischalten", "odblokuj", "subscribe",
                       "про", "премиум", "разблокировать", "безлимит", "подписка",
                       "desbloquear", "ilimitado", "suscribir", "desbloquear", "ilimitado", "assinar",
                       "débloquer", "illimité", "abonnement"],
        "why_bookmaker": ["why bookmaker", "why partner", "why advertise", "why promote",
                          "perché bookmaker", "warum buchmacher", "dlaczego bukmacher",
                          "почему букмекер", "почему партнёр", "зачем реклама",
                          "por qué casa de apuestas", "por qué socio", "por que casa de apostas", "por que parceiro",
                          "pourquoi bookmaker", "pourquoi partenaire"],
        "why_not_free": ["why not free", "why pay", "why limit", "not free", "cost", "price",
                         ...
                         "почему платить", "почему не бесплатно", "цена", "дорого", "деньги",
                         "por qué pagar", "por qué no es gratis", "precio", "caro", "pagar", "gratis", "no es gratis", "cuánto cuesta",
                         "por que pagar", "por que não é grátis", "preço", "grátis", "quanto custa",
                         "pourquoi payer", "pourquoi pas gratuit", "prix", "cher", "payer", "gratuit", "combien"],
        "betting_terms": ["what is 1x2", "what is btts", "over under", "handicap", "accumulator",
                          ...
                          "was bedeutet", "co znaczy", "что значит", "что такое", "объясни",
                          "qué significa", "qué es", "explica", "o que significa", "o que é",
                          "que signifie", "c'est quoi", "explique"],
        "bookmaker_info": ["bookmaker", "deposit", "withdraw", "withdrawal", "payout",
                           ...
                           "букмекер", "депозит", "вывод", "пополнение", "верификация", "лицензия",
                           "casa de apuestas", "depósito", "retirar", "retiro", "verificación", "licencia",
                           "casa de apostas", "sacar", "saque", "verificação",
                           "dépôt", "retrait", "paiement", "vérification", "licence"],
        "password_reset": ["forgot password", "reset password", "can't log in", "lost password",
                          ...
                          "olvidé mi contraseña", "restablecer contraseña", "no puedo iniciar sesión", "cambiar contraseña",
                          "esqueci minha senha", "redefinir senha", "não consigo entrar",
                          "mot de passe oublié", "réinitialiser", "je ne peux pas me connecter"],
        "support_issues": ["problem", "issue", "error", "bug", "can't", "doesn't work",
                           ...
                           "не работает", "ошибка", "не могу", "завис", "не загружается", "не активировал",
                           "no funciona", "no puedo", "atascado", "no carga",
                           "não funciona", "não consigo", "travado", "não carrega",
                           "ne marche pas", "ne fonctionne pas", "erreur", "problème", "bloqué"],
        "beginner_guide": ["beginner", "new to betting", "tips", "guide", "advice",
                           "principiante", "anfänger", "początkujący", "новичок", "советы", "гайд",
                           "nuevo", "consejos", "guía", "iniciante", "novato", "dicas", "guia",
                           "débutant", "conseils", "guide"],
    }
